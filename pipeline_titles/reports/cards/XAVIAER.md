# Respectfully, Xaviaer (@XAVIAER)

Channel group: **right** (title-leaning score, document 14) · organization: PragerU · clipper: no · platform: youtube · subscribers: 195000.0


## videos

Titles: 68 rows, 68 unique (repeat share 0.0%); ranked; political share 97.1%.


Top topics: Black America and Racism Discussions (10.3%); Iran political unrest (7.3%); ICE protests in Minneapolis (5.9%); Islam and Political Extremism (5.9%); Left-Wing Political Violence (4.4%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | 48.9 | 40.1 | 0.2 | 0.2 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | 40.5 | 42.2 | -0.3 | 0.1 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | 74.7 | 63.3 | -0.2 | -0.3 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | 29.5 | 25.3 | -0.3 | -0.1 |
| F5: Question and explainer framing (why, what, ?) | 73.0 | 71.3 | 0.7 | 0.2 |
| F6: Person-centred (named people) | 62.4 | 59.9 | -0.0 | -0.3 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | 65.8 | 65.0 | -0.4 | -0.6 |
| F8: Numeric and dated (digits, years) | 72.2 | 55.7 | 0.0 | -0.1 |
| F9: ALL-CAPS shouting | 24.9 | 20.3 | -0.6 | -0.3 |
| F10: Quoted speech | 13.9 | 38.8 | -0.8 | -0.4 |
| F11: +n_chars +vader_pos +nominalisation +n_org | 35.4 | 39.7 | -0.5 | -0.2 |
| F12: +first_pl +future_will +modal | 9.3 | 16.0 | -0.7 | -0.3 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 0.0% (4.8%); outrage 55.9% (60.1%); humor 0.0% (0.2%); question 25.0% (15.2%); breaking_live 0.0% (2.0%); episode_show 0.0% (7.4%); interview_guest 10.3% (7.8%); reaction 20.6% (2.2%); confrontation 2.9% (8.2%); listicle 0.0% (0.1%); howto_explainer 7.3% (9.3%)


Nearest style neighbors: @MichaelKnowles [right]; @LIVESNEAKO [neutral]; @Forthepeoplepodcast305 [left]; @NovaraMedia [left]; @PTLRadioShow [left]

Nearest topic neighbors: @MichaelKnowles [right]; @BenShapiro [right]; @CamHigby [right]; @ActualJusticeWarrior [right]; @TheAmalaEkpunobi [right]


Engagement: n < 100 titles with views, not estimated.


Lexical diversity: Heaps' beta (1,500 tokens) None, Zipf None, formulaic titles 5.8824 per 100, mean length 9.6176 tokens.


## streams

Titles: 2 rows, 2 unique (repeat share 0.0%); LOW-N (not ranked); political share 100.0%.


Top topics: Black America and Racism Discussions (50.0%); ABC News and Political Media Figures (50.0%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | None | None | 1.3 | 0.1 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | None | None | -1.7 | -0.3 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | None | None | -0.5 | -0.3 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | None | None | 1.0 | -0.1 |
| F5: Question and explainer framing (why, what, ?) | None | None | -0.0 | -0.3 |
| F6: Person-centred (named people) | None | None | -2.4 | -0.1 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | None | None | 0.1 | -0.6 |
| F8: Numeric and dated (digits, years) | None | None | -1.4 | 0.1 |
| F9: ALL-CAPS shouting | None | None | 0.7 | -0.4 |
| F10: Quoted speech | None | None | -0.6 | -0.4 |
| F11: +n_chars +vader_pos +nominalisation +n_org | None | None | -0.3 | 0.1 |
| F12: +first_pl +future_will +modal | None | None | -0.3 | -0.2 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 0.0% (3.9%); outrage 0.0% (55.8%); humor 0.0% (0.1%); question 0.0% (13.0%); breaking_live 0.0% (16.3%); episode_show 0.0% (25.9%); interview_guest 0.0% (14.2%); reaction 0.0% (2.1%); confrontation 0.0% (9.2%); listicle 0.0% (0.1%); howto_explainer 0.0% (5.4%)


Engagement: n < 100 titles with views, not estimated.


Lexical diversity: Heaps' beta (1,500 tokens) None, Zipf None, formulaic titles 0.0 per 100, mean length 5.5 tokens.
