# Sav Says (@SavSays)

Channel group: **right** (title-leaning score, document 14) · organization: Sav Says · clipper: no · platform: youtube · subscribers: 144000.0


## videos

Titles: 16 rows, 16 unique (repeat share 0.0%); LOW-N (not ranked); political share 100.0%.


Top topics: ICE protests in Minneapolis (12.5%); Lindsay Clancy Murder Trial (12.5%); Homelessness in US Cities (12.5%); AI and Political Implications (6.2%); Charlie Kirk Murder Trial (6.2%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | None | None | -0.7 | 0.2 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | None | None | 0.5 | 0.1 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | None | None | -0.1 | -0.3 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | None | None | 0.3 | -0.1 |
| F5: Question and explainer framing (why, what, ?) | None | None | 1.6 | 0.2 |
| F6: Person-centred (named people) | None | None | -1.4 | -0.3 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | None | None | -0.2 | -0.6 |
| F8: Numeric and dated (digits, years) | None | None | 1.0 | -0.1 |
| F9: ALL-CAPS shouting | None | None | 0.2 | -0.3 |
| F10: Quoted speech | None | None | -0.4 | -0.4 |
| F11: +n_chars +vader_pos +nominalisation +n_org | None | None | -0.8 | -0.2 |
| F12: +first_pl +future_will +modal | None | None | -2.0 | -0.3 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 0.0% (4.8%); outrage 75.0% (60.1%); humor 0.0% (0.2%); question 12.5% (15.2%); breaking_live 0.0% (2.0%); episode_show 0.0% (7.4%); interview_guest 0.0% (7.8%); reaction 6.2% (2.2%); confrontation 0.0% (8.2%); listicle 6.2% (0.1%); howto_explainer 25.0% (9.3%)


Engagement: n < 100 titles with views, not estimated.


Lexical diversity: Heaps' beta (1,500 tokens) None, Zipf None, formulaic titles 0.0 per 100, mean length 9.1875 tokens.


## streams

Titles: 1 rows, 1 unique (repeat share 0.0%); LOW-N (not ranked); political share 100.0%.


Top topics: Shocking Events and Reactions (100.0%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | None | None | -0.2 | 0.1 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | None | None | -1.6 | -0.3 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | None | None | -1.8 | -0.3 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | None | None | 2.4 | -0.1 |
| F5: Question and explainer framing (why, what, ?) | None | None | -1.0 | -0.3 |
| F6: Person-centred (named people) | None | None | -0.4 | -0.1 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | None | None | 0.9 | -0.6 |
| F8: Numeric and dated (digits, years) | None | None | -1.0 | 0.1 |
| F9: ALL-CAPS shouting | None | None | -0.3 | -0.4 |
| F10: Quoted speech | None | None | 1.4 | -0.4 |
| F11: +n_chars +vader_pos +nominalisation +n_org | None | None | 2.2 | 0.1 |
| F12: +first_pl +future_will +modal | None | None | -2.7 | -0.2 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 0.0% (3.9%); outrage 0.0% (55.8%); humor 0.0% (0.1%); question 0.0% (13.0%); breaking_live 0.0% (16.3%); episode_show 0.0% (25.9%); interview_guest 0.0% (14.2%); reaction 0.0% (2.1%); confrontation 0.0% (9.2%); listicle 0.0% (0.1%); howto_explainer 0.0% (5.4%)


Engagement: n < 100 titles with views, not estimated.


Lexical diversity: Heaps' beta (1,500 tokens) None, Zipf None, formulaic titles 0.0 per 100, mean length 3.0 tokens.
