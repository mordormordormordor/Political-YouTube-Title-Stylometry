# Lovett or Leave It (@lovettorleaveitpodcast)

Channel group: **left** (title-leaning score, document 14) · organization: Crooked Media · clipper: no · platform: youtube · subscribers: 144000.0


## videos

Titles: 110 rows, 110 unique (repeat share 0.0%); ranked; political share 97.3%.


Top topics: Hollywood and Awards Controversy (4.5%); Trump press conference outbursts (4.5%); Political Comedy and Media (4.5%); ICE protests in Minneapolis (3.6%); Trump and Iran War Negotiations (3.6%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | 7.6 | 12.7 | -0.8 | 0.1 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | 33.1 | 25.4 | -0.4 | -0.1 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | 1.7 | 1.3 | -0.8 | -0.3 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | 8.9 | 13.1 | -0.4 | -0.1 |
| F5: Question and explainer framing (why, what, ?) | 53.8 | 48.7 | 0.2 | 0.1 |
| F6: Person-centred (named people) | 96.2 | 97.0 | 1.2 | -0.2 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | 32.6 | 25.4 | -0.7 | -0.5 |
| F8: Numeric and dated (digits, years) | 35.6 | 37.7 | -0.3 | -0.2 |
| F9: ALL-CAPS shouting | 5.9 | 8.5 | -1.0 | -0.3 |
| F10: Quoted speech | 22.5 | 32.2 | -0.7 | -0.4 |
| F11: +n_chars +vader_pos +nominalisation +n_org | 76.3 | 75.4 | 0.3 | -0.3 |
| F12: +first_pl +future_will +modal | 33.1 | 28.4 | -0.4 | -0.1 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 2.7% (3.6%); outrage 47.3% (63.9%); humor 0.9% (0.2%); question 14.5% (15.6%); breaking_live 0.0% (2.9%); episode_show 0.0% (2.8%); interview_guest 16.4% (9.8%); reaction 10.0% (2.9%); confrontation 21.8% (7.7%); listicle 0.0% (0.1%); howto_explainer 11.8% (8.0%)


Nearest style neighbors: @TheAdamCarollaShow1 [right]; @thejimmydoreshow [neutral]; @MyronGainesX [right]; @PTLRadioShow [left]; @marclamonthillnetwork [left]

Nearest topic neighbors: @dollemore [left]; @AfterPartyEmily [right]; @podsaveamerica [left]; @DueDissidence [left]; @SMN [left]


Monthly drift (topic-controlled scores and hook shares; * = partial month):

| month | n | F1 | F2 | F3 | F4 | F5 | F6 | F7 | F8 | F9 | F10 | F11 | F12 | curiosity_gap | outrage | humor |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 2026-06 | 17 | -0.69 | -1.40 | -1.05 | -0.68 | -0.37 | 0.85 | -0.69 | -0.58 | -0.99 | -0.98 | 0.99 | -0.77 | 0.06 | 0.41 | 0.00 |
| 2026-07 | 16 | -0.08 | -1.02 | -1.16 | -0.43 | 0.07 | 0.69 | -1.12 | -0.39 | -1.43 | -1.01 | 0.83 | -1.02 | 0.00 | 0.50 | 0.00 |


Engagement (n = 110, R2 = 0.4592; log views per within-creator SD, month + topic controls):

| predictor | coef_per_sd | se | p |
|---|---|---|---|
| F1 | -0.316 | None | 1.000 |
| F2 | -0.038 | None | 1.000 |
| F3 | -0.148 | None | 1.000 |
| F4 | 0.088 | None | 1.000 |
| F5 | 0.042 | None | 1.000 |
| F6 | -0.203 | None | 1.000 |
| F7 | 0.025 | None | 1.000 |
| F8 | 0.041 | None | 1.000 |
| F9 | 0.300 | None | 1.000 |
| F10 | -0.248 | None | 1.000 |
| F11 | 0.133 | None | 1.000 |
| F12 | -0.204 | None | 1.000 |
| curiosity_gap | 0.023 | None | 1.000 |
| outrage | 0.128 | None | 1.000 |
| humor | 0.050 | None | 1.000 |
| n_tokens | 0.150 | None | 1.000 |


Hit concentration: Gini 0.4959, top-10 % share 39.7%, power-law tail not supported vs lognormal (LR = -0.3847, p = 0.7005).


Lexical diversity: Heaps' beta (1,500 tokens) None, Zipf None, formulaic titles 3.6364 per 100, mean length 11.3818 tokens.
