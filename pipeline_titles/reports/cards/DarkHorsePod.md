# Bret Weinstein (@DarkHorsePod)

Channel group: **right** (title-leaning score, document 14) · organization: DarkHorse · clipper: no · platform: youtube · subscribers: 551000.0


## videos

Titles: 65 rows, 65 unique (repeat share 0.0%); ranked; political share 87.7%.


Top topics: Covid Vaccine Controversy (7.7%); Islam and Political Extremism (4.6%); Health Industry and Big Pharma (4.6%); UFO Government Disclosure (4.6%); Books and Cultural Commentary (4.6%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | 62.4 | 82.3 | 0.4 | 0.2 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | 8.4 | 10.5 | -1.1 | 0.1 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | 92.8 | 91.1 | 0.2 | -0.3 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | 63.3 | 59.1 | -0.0 | -0.1 |
| F5: Question and explainer framing (why, what, ?) | 69.6 | 76.8 | 0.6 | 0.2 |
| F6: Person-centred (named people) | 78.5 | 65.4 | 0.3 | -0.3 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | 23.6 | 22.4 | -0.9 | -0.6 |
| F8: Numeric and dated (digits, years) | 67.9 | 75.9 | -0.0 | -0.1 |
| F9: ALL-CAPS shouting | 46.0 | 40.9 | -0.3 | -0.3 |
| F10: Quoted speech | 66.2 | 66.2 | -0.1 | -0.4 |
| F11: +n_chars +vader_pos +nominalisation +n_org | 47.7 | 54.4 | -0.3 | -0.2 |
| F12: +first_pl +future_will +modal | 53.2 | 42.6 | -0.1 | -0.3 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 3.1% (4.8%); outrage 13.9% (60.1%); humor 0.0% (0.2%); question 21.5% (15.2%); breaking_live 0.0% (2.0%); episode_show 1.5% (7.4%); interview_guest 6.2% (7.8%); reaction 3.1% (2.2%); confrontation 0.0% (8.2%); listicle 0.0% (0.1%); howto_explainer 12.3% (9.3%)


Nearest style neighbors: @MichaelKnowles [right]; @TheJoyReidShow [left]; @DropSiteNews [left]; @underthedesknews [left]; @ClubRandomPodcast [neutral]

Nearest topic neighbors: @ZubyMusic [right]; @morebridgetphetasy [right]; @PragerU [right]; @FleccasTalks [right]; @fightbackpodcast [right]


Engagement: n < 100 titles with views, not estimated.


Lexical diversity: Heaps' beta (1,500 tokens) None, Zipf None, formulaic titles 26.1538 per 100, mean length 9.2923 tokens.


## streams

Titles: 35 rows, 35 unique (repeat share 0.0%); LOW-N (not ranked); political share 94.3%.


Top topics: Books and Cultural Commentary (48.6%); Epstein Files Revelations (5.7%); Feminism and Gender Roles Debate (5.7%); American History and Political Threats (5.7%); Covid Vaccine Controversy (5.7%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | None | None | 1.0 | 0.1 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | None | None | -1.0 | -0.3 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | None | None | 0.5 | -0.3 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | None | None | 0.2 | -0.1 |
| F5: Question and explainer framing (why, what, ?) | None | None | 1.3 | -0.3 |
| F6: Person-centred (named people) | None | None | 3.5 | -0.1 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | None | None | -0.1 | -0.6 |
| F8: Numeric and dated (digits, years) | None | None | 1.0 | 0.1 |
| F9: ALL-CAPS shouting | None | None | 1.2 | -0.4 |
| F10: Quoted speech | None | None | -1.0 | -0.4 |
| F11: +n_chars +vader_pos +nominalisation +n_org | None | None | 0.9 | 0.1 |
| F12: +first_pl +future_will +modal | None | None | 0.4 | -0.2 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 0.0% (3.9%); outrage 0.0% (55.8%); humor 0.0% (0.1%); question 28.6% (13.0%); breaking_live 0.0% (16.3%); episode_show 0.0% (25.9%); interview_guest 97.1% (14.2%); reaction 0.0% (2.1%); confrontation 0.0% (9.2%); listicle 0.0% (0.1%); howto_explainer 5.7% (5.4%)


Engagement: n < 100 titles with views, not estimated.


Lexical diversity: Heaps' beta (1,500 tokens) None, Zipf None, formulaic titles 94.2857 per 100, mean length 14.0 tokens.
