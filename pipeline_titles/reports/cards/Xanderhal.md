# Xanderhal (@Xanderhal)

Channel group: **left** (title-leaning score, document 14) · organization: Xanderhal · clipper: no · platform: youtube · subscribers: 114000.0


## videos

Titles: 289 rows, 289 unique (repeat share 0.0%); ranked; political share 91.7%.


Top topics: Left-Wing Political Violence (6.6%); Transgender Rights and Political Debate (5.5%); Shocking Events and Reactions (4.8%); Woke Culture Evolution (4.2%); MAGA Movement Crisis (3.8%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | 32.9 | 37.1 | -0.1 | 0.1 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | 82.3 | 84.4 | 0.7 | -0.1 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | 18.1 | 9.7 | -0.5 | -0.3 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | 78.9 | 81.4 | 0.1 | -0.1 |
| F5: Question and explainer framing (why, what, ?) | 36.3 | 44.3 | -0.1 | 0.1 |
| F6: Person-centred (named people) | 44.7 | 32.9 | -0.3 | -0.2 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | 32.9 | 26.2 | -0.7 | -0.5 |
| F8: Numeric and dated (digits, years) | 21.5 | 12.7 | -0.4 | -0.2 |
| F9: ALL-CAPS shouting | 35.4 | 53.2 | -0.5 | -0.3 |
| F10: Quoted speech | 69.2 | 60.3 | -0.1 | -0.4 |
| F11: +n_chars +vader_pos +nominalisation +n_org | 11.8 | 8.4 | -1.0 | -0.3 |
| F12: +first_pl +future_will +modal | 24.5 | 12.7 | -0.5 | -0.1 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 4.8% (3.6%); outrage 80.3% (63.9%); humor 0.7% (0.2%); question 9.0% (15.6%); breaking_live 0.0% (2.9%); episode_show 0.0% (2.8%); interview_guest 3.1% (9.8%); reaction 0.7% (2.9%); confrontation 4.2% (7.7%); listicle 0.0% (0.1%); howto_explainer 2.4% (8.0%)


Nearest style neighbors: @CamHigby [right]; @RileyGaines [right]; @TheMichaelCohenShow [left]; @SaltyCracker [right]; @TheAmalaEkpunobi [right]

Nearest topic neighbors: @TimcastIRL [right]; @MichaelKnowles [right]; @TheSerfTimes [left]; @MattWalsh [right]; @TheVaushPit [left]


Monthly drift (topic-controlled scores and hook shares; * = partial month):

| month | n | F1 | F2 | F3 | F4 | F5 | F6 | F7 | F8 | F9 | F10 | F11 | F12 | curiosity_gap | outrage | humor |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 2026-01 | 24 | 0.51 | 1.71 | -0.53 | -0.17 | 0.41 | -0.17 | -0.63 | -0.08 | -0.04 | 0.48 | -0.67 | -0.86 | 0.00 | 0.79 | 0.00 |
| 2026-03 | 23 | -0.25 | 0.50 | -0.39 | 1.00 | 0.16 | -0.76 | -0.86 | -0.54 | -0.33 | -0.97 | -1.34 | -1.04 | 0.04 | 0.87 | 0.00 |
| 2026-04 | 27 | -1.00 | 0.04 | -0.42 | -0.07 | -0.57 | 0.23 | -0.16 | -0.51 | -0.77 | -0.39 | -0.49 | -1.02 | 0.07 | 0.74 | 0.00 |
| 2026-05 | 25 | -0.07 | 0.90 | -0.45 | 0.56 | 0.27 | -0.82 | -0.68 | -0.52 | -0.03 | -0.60 | -1.02 | 0.03 | 0.12 | 0.92 | 0.04 |
| 2026-06 | 88 | 0.17 | 0.33 | -0.41 | -0.05 | -0.30 | -0.32 | -0.87 | -0.24 | -0.37 | -0.12 | -0.84 | -0.35 | 0.05 | 0.75 | 0.00 |
| 2026-07 | 52 | -0.40 | 0.90 | -0.70 | 0.03 | -0.27 | -0.37 | -0.97 | -0.66 | -0.61 | 0.83 | -1.25 | -0.16 | 0.04 | 0.85 | 0.02 |
| 2026-08 | 18 | -0.50 | 0.88 | -0.46 | 1.17 | 0.10 | -1.66 | -0.47 | -0.71 | -1.22 | -0.56 | -0.61 | -0.90 | 0.06 | 0.78 | 0.00 |
| 2026-09* | 29 | 0.16 | 0.77 | -0.42 | -0.29 | 0.21 | 0.52 | -0.70 | 0.07 | -0.30 | -0.37 | -1.16 | -0.26 | 0.03 | 0.79 | 0.00 |


Engagement (n = 289, R2 = 0.318; log views per within-creator SD, month + topic controls):

| predictor | coef_per_sd | se | p |
|---|---|---|---|
| F1 | 0.027 | 0.057 | 0.633 |
| F2 | 0.069 | 0.055 | 0.212 |
| F3 | -0.077 | 0.051 | 0.130 |
| F4 | 0.047 | 0.054 | 0.388 |
| F5 | -0.042 | 0.046 | 0.355 |
| F6 | -0.034 | 0.060 | 0.573 |
| F7 | -0.073 | 0.047 | 0.121 |
| F8 | -0.020 | 0.045 | 0.650 |
| F9 | -0.062 | 0.058 | 0.288 |
| F10 | -0.041 | 0.054 | 0.446 |
| F11 | -0.019 | 0.055 | 0.734 |
| F12 | -0.090 | 0.061 | 0.139 |
| curiosity_gap | 0.027 | 0.060 | 0.654 |
| outrage | -0.023 | 0.050 | 0.649 |
| humor | -0.031 | 0.070 | 0.652 |
| n_tokens | -0.009 | 0.059 | 0.885 |


Hit concentration: Gini 0.4399, top-10 % share 34.9%, power-law tail not supported vs lognormal (LR = -1.4753, p = 0.1401).


Lexical diversity: Heaps' beta (1,500 tokens) 0.8351, Zipf 0.6762, formulaic titles 10.3806 per 100, mean length 8.4394 tokens.


## streams

Titles: 101 rows, 101 unique (repeat share 0.0%); ranked; political share 94.1%.


Top topics: Iran and Political Updates (36.6%); Transgender Rights and Political Debate (6.9%); Media and Journalism Critique (4.0%); Ben Shapiro and Political Media (4.0%); Asmongold controversy and bans (3.0%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | 31.2 | 35.1 | -0.5 | -0.2 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | 94.8 | 89.6 | 1.0 | -0.4 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | 14.3 | 5.2 | -0.6 | -0.2 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | 81.8 | 79.2 | 0.1 | -0.2 |
| F5: Question and explainer framing (why, what, ?) | 24.7 | 28.6 | -0.5 | -0.3 |
| F6: Person-centred (named people) | 71.4 | 81.8 | 0.8 | 0.7 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | 90.9 | 83.1 | 1.2 | -0.4 |
| F8: Numeric and dated (digits, years) | 84.4 | 83.1 | 0.5 | -0.1 |
| F9: ALL-CAPS shouting | 97.4 | 97.4 | 4.3 | -0.4 |
| F10: Quoted speech | 96.1 | 84.4 | 0.4 | -0.6 |
| F11: +n_chars +vader_pos +nominalisation +n_org | 75.3 | 75.3 | 0.5 | 0.0 |
| F12: +first_pl +future_will +modal | 14.3 | 10.4 | -0.6 | -0.0 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 3.0% (2.6%); outrage 90.1% (62.7%); humor 0.0% (0.1%); question 1.0% (11.3%); breaking_live 0.0% (17.4%); episode_show 0.0% (6.2%); interview_guest 2.0% (18.4%); reaction 0.0% (1.4%); confrontation 19.8% (15.4%); listicle 0.0% (0.0%); howto_explainer 1.0% (2.8%)


Nearest style neighbors: @RealAmericasVoice [right]; @SkyNews [left]; @deanwithrs [left]; @DueDissidence [left]; @SabbySabs [left]

Nearest topic neighbors: @SabbySabs [left]; @MyronGainesX [right]; @chicksonright [right]; @DueDissidence [left]; @MLChristiansen [right]


Monthly drift (topic-controlled scores and hook shares; * = partial month):

| month | n | F1 | F2 | F3 | F4 | F5 | F6 | F7 | F8 | F9 | F10 | F11 | F12 | curiosity_gap | outrage | humor |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 2026-06 | 21 | -1.06 | 1.48 | -0.10 | -0.29 | -0.76 | 1.81 | 1.58 | 0.28 | 4.09 | 0.35 | -0.09 | -0.13 | 0.00 | 0.90 | 0.00 |
| 2026-07 | 17 | -1.03 | 0.45 | -0.65 | -0.05 | -0.56 | 1.26 | 0.91 | -0.11 | 4.30 | 1.80 | -0.40 | -0.68 | 0.00 | 0.88 | 0.00 |


Engagement: n < 100 titles with views, not estimated.


Lexical diversity: Heaps' beta (1,500 tokens) None, Zipf None, formulaic titles 20.7921 per 100, mean length 11.8713 tokens.
