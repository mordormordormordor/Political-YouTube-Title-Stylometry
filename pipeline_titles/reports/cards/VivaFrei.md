# Viva Frei (@VivaFrei)

Channel group: **right** (title-leaning score, document 14) · organization: Viva Frei · clipper: no · platform: youtube · subscribers: 653000.0


## videos

Titles: 310 rows, 310 unique (repeat share 0.0%); ranked; political share 97.4%.


Top topics: Alberta independence movement (7.7%); Lindsay Clancy Murder Trial (4.8%); Charlie Kirk Murder Trial (4.2%); Karmelo Anthony Trial Verdict (4.2%); Covid Vaccine Controversy (3.9%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | 22.0 | 17.8 | -0.3 | 0.2 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | 63.6 | 61.0 | 0.2 | 0.1 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | 71.6 | 65.7 | -0.2 | -0.3 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | 22.9 | 15.7 | -0.3 | -0.1 |
| F5: Question and explainer framing (why, what, ?) | 60.2 | 54.7 | 0.3 | 0.2 |
| F6: Person-centred (named people) | 92.8 | 93.6 | 0.9 | -0.3 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | 11.4 | 20.8 | -1.1 | -0.7 |
| F8: Numeric and dated (digits, years) | 86.4 | 82.2 | 0.2 | -0.1 |
| F9: ALL-CAPS shouting | 53.0 | 42.4 | -0.3 | -0.3 |
| F10: Quoted speech | 92.4 | 90.3 | 0.9 | -0.4 |
| F11: +n_chars +vader_pos +nominalisation +n_org | 97.9 | 95.3 | 1.3 | -0.2 |
| F12: +first_pl +future_will +modal | 19.9 | 23.3 | -0.6 | -0.3 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 3.2% (4.8%); outrage 79.3% (59.8%); humor 0.3% (0.2%); question 36.8% (15.3%); breaking_live 2.6% (2.0%); episode_show 0.0% (7.5%); interview_guest 20.0% (7.7%); reaction 0.7% (2.2%); confrontation 7.7% (7.8%); listicle 0.0% (0.1%); howto_explainer 18.4% (9.3%)


Nearest style neighbors: @MegynKelly [right]; @rolandsmartin [left]; @TomiLahrenIsFearless [right]; @MLChristiansen [right]; https://rumble.com/c/GGreenwald [left]

Nearest topic neighbors: @MegynKelly [right]; @RealAmericasVoice [right]; @NewsmaxTV [right]; @bennyjohnson [right]; @BlazeTV [right]


Monthly drift (topic-controlled scores and hook shares; * = partial month):

| month | n | F1 | F2 | F3 | F4 | F5 | F6 | F7 | F8 | F9 | F10 | F11 | F12 | curiosity_gap | outrage | humor |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 2026-01 | 37 | -0.38 | 0.11 | -0.28 | -0.40 | -0.03 | 1.01 | -1.38 | 0.50 | -0.47 | 0.48 | 1.41 | -0.32 | 0.00 | 0.92 | 0.00 |
| 2026-02 | 37 | -1.76 | 0.47 | -0.11 | -0.15 | 0.34 | 0.93 | -0.85 | -0.10 | -0.26 | 1.10 | 0.71 | -0.96 | 0.03 | 0.92 | 0.00 |
| 2026-03 | 36 | -0.70 | 0.70 | -0.08 | -0.10 | -0.29 | -0.17 | -1.05 | 0.47 | -0.39 | 1.74 | 0.79 | -0.16 | 0.06 | 0.78 | 0.03 |
| 2026-04 | 38 | 0.27 | -0.09 | -0.38 | -0.42 | 1.13 | 0.94 | -1.15 | 0.90 | -0.36 | 0.43 | 1.28 | -0.60 | 0.05 | 0.82 | 0.00 |
| 2026-05 | 39 | -0.15 | 0.15 | -0.30 | -0.51 | 0.07 | 0.78 | -0.75 | -0.07 | -0.30 | 0.87 | 1.27 | -0.45 | 0.03 | 0.72 | 0.00 |
| 2026-06 | 37 | -0.07 | 0.16 | 0.12 | 0.07 | 0.45 | 1.46 | -1.42 | -0.07 | 0.66 | 1.49 | 1.44 | -0.45 | 0.05 | 0.84 | 0.00 |
| 2026-07 | 25 | 0.67 | 0.07 | -0.19 | -0.10 | 0.40 | 1.65 | -0.74 | 0.15 | -0.74 | 0.86 | 2.00 | -1.19 | 0.00 | 0.64 | 0.00 |
| 2026-08 | 39 | 0.64 | 0.19 | -0.19 | -0.49 | 0.46 | 0.87 | -1.12 | 0.04 | -0.53 | 0.64 | 2.05 | -0.58 | 0.05 | 0.72 | 0.00 |
| 2026-09* | 22 | -1.72 | 0.32 | -0.26 | -0.41 | 0.21 | 0.30 | -1.18 | 0.32 | -0.09 | 0.97 | 0.74 | -0.41 | 0.00 | 0.73 | 0.00 |


Engagement (n = 310, R2 = 0.3703; log views per within-creator SD, month + topic controls):

| predictor | coef_per_sd | se | p |
|---|---|---|---|
| F1 | -0.094 | 0.090 | 0.298 |
| F2 | 0.108 | 0.052 | 0.036 |
| F3 | -0.003 | 0.053 | 0.955 |
| F4 | -0.002 | 0.068 | 0.973 |
| F5 | -0.013 | 0.057 | 0.822 |
| F6 | 0.098 | 0.059 | 0.096 |
| F7 | -0.141 | 0.068 | 0.038 |
| F8 | 0.040 | 0.055 | 0.464 |
| F9 | -0.055 | 0.054 | 0.308 |
| F10 | -0.036 | 0.050 | 0.475 |
| F11 | 0.155 | 0.081 | 0.054 |
| F12 | -0.095 | 0.054 | 0.077 |
| curiosity_gap | 0.038 | 0.059 | 0.516 |
| outrage | 0.159 | 0.053 | 0.002 |
| humor | 0.093 | 0.852 | 0.913 |
| n_tokens | -0.114 | 0.053 | 0.034 |


Hit concentration: Gini 0.4986, top-10 % share 39.0%, power-law tail not supported vs lognormal (LR = -0.008, p = 0.9936).


Lexical diversity: Heaps' beta (1,500 tokens) 0.8708, Zipf 0.6074, formulaic titles 18.3871 per 100, mean length 14.4484 tokens.


## streams

Titles: 49 rows, 49 unique (repeat share 0.0%); LOW-N (not ranked); political share 100.0%.


Top topics: Iran and Political Updates (32.6%); Alberta independence movement (6.1%); Supreme Court rulings and controversies (6.1%); Lindsay Clancy Murder Trial (4.1%); Joe Kent Resignation and Leaks (4.1%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | None | None | 0.2 | 0.1 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | None | None | -0.5 | -0.3 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | None | None | -0.2 | -0.3 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | None | None | 0.4 | -0.1 |
| F5: Question and explainer framing (why, what, ?) | None | None | -0.2 | -0.3 |
| F6: Person-centred (named people) | None | None | 1.3 | -0.1 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | None | None | -0.4 | -0.6 |
| F8: Numeric and dated (digits, years) | None | None | -0.0 | 0.1 |
| F9: ALL-CAPS shouting | None | None | 0.4 | -0.4 |
| F10: Quoted speech | None | None | -0.6 | -0.4 |
| F11: +n_chars +vader_pos +nominalisation +n_org | None | None | 1.4 | 0.1 |
| F12: +first_pl +future_will +modal | None | None | -0.6 | -0.2 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 0.0% (3.9%); outrage 57.1% (55.8%); humor 0.0% (0.1%); question 36.7% (13.0%); breaking_live 12.2% (16.3%); episode_show 71.4% (25.9%); interview_guest 20.4% (14.2%); reaction 0.0% (2.1%); confrontation 8.2% (9.2%); listicle 0.0% (0.1%); howto_explainer 2.0% (5.4%)


Engagement: n < 100 titles with views, not estimated.


Lexical diversity: Heaps' beta (1,500 tokens) None, Zipf None, formulaic titles 57.1429 per 100, mean length 12.8776 tokens.
