# Styxhexenhammer666 (@Styxhexenhammer666)

Channel group: **right** (title-leaning score, document 14) · organization: Styxhexenhammer · clipper: no · platform: youtube · subscribers: 484000.0


## videos

Titles: 383 rows, 380 unique (repeat share 0.8%); ranked; political share 90.0%.


Top topics: Health Industry and Big Pharma (6.0%); Trump's Beef and Food Policy Impact (4.2%); Trump and Iran War Negotiations (2.9%); Trump vs Maduro in Venezuela (2.9%); Left-Wing Political Violence (2.9%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | 88.6 | 87.7 | 0.8 | 0.2 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | 21.6 | 25.0 | -0.6 | 0.1 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | 16.1 | 27.1 | -0.5 | -0.3 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | 43.6 | 48.7 | -0.1 | -0.1 |
| F5: Question and explainer framing (why, what, ?) | 19.5 | 31.4 | -0.3 | 0.2 |
| F6: Person-centred (named people) | 34.3 | 28.8 | -0.5 | -0.3 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | 16.9 | 23.7 | -1.0 | -0.7 |
| F8: Numeric and dated (digits, years) | 20.8 | 36.0 | -0.4 | -0.1 |
| F9: ALL-CAPS shouting | 28.8 | 30.5 | -0.5 | -0.3 |
| F10: Quoted speech | 8.5 | 22.9 | -0.9 | -0.4 |
| F11: +n_chars +vader_pos +nominalisation +n_org | 20.3 | 32.6 | -0.8 | -0.2 |
| F12: +first_pl +future_will +modal | 58.9 | 65.3 | -0.1 | -0.3 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 3.7% (4.8%); outrage 49.7% (59.8%); humor 0.5% (0.2%); question 8.9% (15.3%); breaking_live 1.1% (2.0%); episode_show 0.0% (7.5%); interview_guest 4.5% (7.7%); reaction 0.3% (2.2%); confrontation 2.9% (7.8%); listicle 0.0% (0.1%); howto_explainer 2.9% (9.3%)


Nearest style neighbors: @ThePodcastoftheLotusEaters [right]; @hutch [neutral]; @AlexStein99 [right]; @franifio [left]; @DylanBurnsLIVE [left]

Nearest topic neighbors: @RealAmericasVoice [right]; @FoxNewsChannelClips [right]; @NewsmaxTV [right]; @NYTPodcasts [left]; @JillianMichaels [right]


Monthly drift (topic-controlled scores and hook shares; * = partial month):

| month | n | F1 | F2 | F3 | F4 | F5 | F6 | F7 | F8 | F9 | F10 | F11 | F12 | curiosity_gap | outrage | humor |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 2026-01 | 92 | 0.77 | -0.51 | -0.42 | -0.22 | 0.01 | -0.40 | -0.89 | -0.51 | -0.42 | -0.88 | -0.69 | -0.09 | 0.09 | 0.50 | 0.00 |
| 2026-02 | 105 | 0.78 | -0.13 | -0.42 | 0.08 | -0.32 | -0.35 | -0.97 | -0.27 | -0.50 | -0.56 | -0.39 | -0.09 | 0.02 | 0.50 | 0.02 |
| 2026-03 | 16 | 1.30 | 0.64 | -0.85 | -0.38 | -0.62 | -0.81 | -2.07 | -0.72 | -0.36 | -0.45 | -0.29 | 1.50 | 0.06 | 0.44 | 0.00 |
| 2026-04 | 58 | 0.78 | -1.14 | -0.68 | -0.13 | -0.18 | -0.44 | -1.11 | -0.43 | -0.60 | -0.94 | -1.20 | -0.21 | 0.05 | 0.57 | 0.00 |
| 2026-05 | 45 | 0.95 | -0.39 | -0.51 | 0.06 | -0.66 | -0.45 | -0.78 | -0.50 | -0.42 | -1.20 | -0.36 | -0.45 | 0.00 | 0.58 | 0.00 |
| 2026-06 | 37 | 0.68 | -0.89 | -0.70 | -0.30 | -0.47 | -0.70 | -1.00 | -0.47 | -0.45 | -0.97 | -1.19 | 0.03 | 0.00 | 0.46 | 0.00 |
| 2026-07 | 21 | 0.32 | -2.60 | -0.32 | -0.79 | -0.96 | -0.78 | -0.43 | 0.32 | -1.09 | -1.26 | -2.10 | -0.42 | 0.00 | 0.29 | 0.00 |


Engagement (n = 380, R2 = 0.3883; log views per within-creator SD, month + topic controls):

| predictor | coef_per_sd | se | p |
|---|---|---|---|
| F1 | -0.012 | 0.042 | 0.780 |
| F2 | 0.067 | 0.041 | 0.097 |
| F3 | 0.011 | 0.043 | 0.795 |
| F4 | 0.021 | 0.037 | 0.572 |
| F5 | -0.032 | 0.042 | 0.441 |
| F6 | 0.064 | 0.036 | 0.073 |
| F7 | -0.064 | 0.037 | 0.084 |
| F8 | -0.011 | 0.023 | 0.642 |
| F9 | -0.047 | 0.042 | 0.255 |
| F10 | -0.022 | 0.033 | 0.507 |
| F11 | -0.016 | 0.054 | 0.763 |
| F12 | -0.060 | 0.046 | 0.186 |
| curiosity_gap | 0.008 | 0.043 | 0.859 |
| outrage | 0.068 | 0.036 | 0.057 |
| humor | -0.041 | 0.018 | 0.020 |
| n_tokens | -0.059 | 0.050 | 0.237 |


Hit concentration: Gini 0.3971, top-10 % share 31.5%, power-law tail not supported vs lognormal (LR = -1.1824, p = 0.237).


Lexical diversity: Heaps' beta (1,500 tokens) 0.8753, Zipf 0.6764, formulaic titles 20.0 per 100, mean length 8.0763 tokens.


## streams

Titles: 16 rows, 16 unique (repeat share 0.0%); LOW-N (not ranked); political share 93.8%.


Top topics: Iran and Political Updates (25.0%); Trump vs Maduro in Venezuela (12.5%); Political Podcasts and Media (12.5%); American History and Political Threats (6.2%); Trump and Greenland politics (6.2%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | None | None | 0.0 | 0.1 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | None | None | -2.4 | -0.3 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | None | None | 0.3 | -0.3 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | None | None | -0.4 | -0.1 |
| F5: Question and explainer framing (why, what, ?) | None | None | -0.5 | -0.3 |
| F6: Person-centred (named people) | None | None | 1.3 | -0.1 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | None | None | -0.4 | -0.6 |
| F8: Numeric and dated (digits, years) | None | None | -0.4 | 0.1 |
| F9: ALL-CAPS shouting | None | None | -1.3 | -0.4 |
| F10: Quoted speech | None | None | -0.9 | -0.4 |
| F11: +n_chars +vader_pos +nominalisation +n_org | None | None | -0.1 | 0.1 |
| F12: +first_pl +future_will +modal | None | None | 0.3 | -0.2 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 0.0% (3.9%); outrage 18.8% (55.8%); humor 0.0% (0.1%); question 0.0% (13.0%); breaking_live 100.0% (16.3%); episode_show 0.0% (25.9%); interview_guest 0.0% (14.2%); reaction 0.0% (2.1%); confrontation 6.2% (9.2%); listicle 0.0% (0.1%); howto_explainer 0.0% (5.4%)


Engagement: n < 100 titles with views, not estimated.


Lexical diversity: Heaps' beta (1,500 tokens) None, Zipf None, formulaic titles 100.0 per 100, mean length 9.875 tokens.
