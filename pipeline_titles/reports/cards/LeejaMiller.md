# Leeja Miller (@LeejaMiller)

Channel group: **left** (title-leaning score, document 14) · organization: Leeja Miller · clipper: no · platform: youtube · subscribers: 708000.0


## videos

Titles: 56 rows, 56 unique (repeat share 0.0%); ranked; political share 94.6%.


Top topics: American History and Political Threats (10.7%); ICE protests in Minneapolis (8.9%); Billionaire Influence in Politics (5.4%); Trump vs Maduro in Venezuela (3.6%); White House Correspondents Dinner Shooting (3.6%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | 81.0 | 75.1 | 0.6 | 0.1 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | 67.5 | 62.9 | 0.3 | -0.1 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | 56.5 | 62.9 | -0.3 | -0.3 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | 83.1 | 79.7 | 0.2 | -0.1 |
| F5: Question and explainer framing (why, what, ?) | 92.0 | 89.9 | 1.6 | 0.1 |
| F6: Person-centred (named people) | 11.8 | 13.5 | -0.8 | -0.2 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | 7.6 | 8.9 | -1.1 | -0.5 |
| F8: Numeric and dated (digits, years) | 25.7 | 31.2 | -0.4 | -0.2 |
| F9: ALL-CAPS shouting | 71.3 | 66.2 | -0.1 | -0.3 |
| F10: Quoted speech | 10.1 | 16.5 | -0.8 | -0.4 |
| F11: +n_chars +vader_pos +nominalisation +n_org | 13.1 | 26.6 | -0.9 | -0.3 |
| F12: +first_pl +future_will +modal | 84.4 | 75.5 | 0.3 | -0.1 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 12.5% (3.6%); outrage 44.6% (63.9%); humor 0.0% (0.2%); question 32.1% (15.6%); breaking_live 0.0% (2.9%); episode_show 0.0% (2.8%); interview_guest 5.4% (9.8%); reaction 0.0% (2.9%); confrontation 0.0% (7.7%); listicle 0.0% (0.1%); howto_explainer 30.4% (8.0%)


Nearest style neighbors: @theisabelbrown [right]; @ajplus [left]; @thomhartmann [left]; @KimIversen [neutral]; @TheAtlantic [left]

Nearest topic neighbors: @thomhartmann [left]; @SMN [left]; @RealAmericasVoice [right]; @cafedotcom [left]; @msnow [left]


Engagement: n < 100 titles with views, not estimated.


Lexical diversity: Heaps' beta (1,500 tokens) None, Zipf None, formulaic titles 12.5 per 100, mean length 7.0179 tokens.


## streams

Titles: 1 rows, 1 unique (repeat share 0.0%); LOW-N (not ranked); political share 100.0%.


Top topics: America 250th Anniversary Celebrations (100.0%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | None | None | -0.0 | -0.2 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | None | None | -1.4 | -0.4 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | None | None | 0.6 | -0.2 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | None | None | 0.4 | -0.2 |
| F5: Question and explainer framing (why, what, ?) | None | None | 0.5 | -0.3 |
| F6: Person-centred (named people) | None | None | -1.8 | 0.7 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | None | None | 0.5 | -0.4 |
| F8: Numeric and dated (digits, years) | None | None | -2.3 | -0.1 |
| F9: ALL-CAPS shouting | None | None | 1.7 | -0.4 |
| F10: Quoted speech | None | None | -0.7 | -0.6 |
| F11: +n_chars +vader_pos +nominalisation +n_org | None | None | -0.9 | 0.0 |
| F12: +first_pl +future_will +modal | None | None | -2.3 | -0.0 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 100.0% (2.6%); outrage 0.0% (62.7%); humor 0.0% (0.1%); question 0.0% (11.3%); breaking_live 0.0% (17.4%); episode_show 0.0% (6.2%); interview_guest 0.0% (18.4%); reaction 0.0% (1.4%); confrontation 0.0% (15.4%); listicle 0.0% (0.0%); howto_explainer 0.0% (2.8%)


Engagement: n < 100 titles with views, not estimated.


Lexical diversity: Heaps' beta (1,500 tokens) None, Zipf None, formulaic titles 0.0 per 100, mean length 5.0 tokens.
