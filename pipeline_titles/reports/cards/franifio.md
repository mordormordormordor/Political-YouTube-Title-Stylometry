# The Bitchuation Room (with Francesca Fiorentini) (@franifio)

Channel group: **left** (title-leaning score, document 14) · organization: Bitchuation Room · clipper: no · platform: youtube · subscribers: 286000.0


## videos

Titles: 271 rows, 271 unique (repeat share 0.0%); ranked; political share 98.5%.


Top topics: Israel and Palestine Conflict (7.0%); Hasan Piker Controversy (3.7%); ICE protests in Minneapolis (3.3%); FIFA World Cup and Trump controversy (2.6%); Immigration and Deportation Policies (2.6%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | 74.7 | 66.7 | 0.5 | 0.1 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | 37.6 | 35.4 | -0.3 | -0.1 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | 14.8 | 15.6 | -0.5 | -0.3 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | 36.3 | 32.5 | -0.2 | -0.1 |
| F5: Question and explainer framing (why, what, ?) | 74.3 | 68.8 | 0.7 | 0.1 |
| F6: Person-centred (named people) | 36.7 | 51.9 | -0.4 | -0.2 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | 40.9 | 44.7 | -0.7 | -0.5 |
| F8: Numeric and dated (digits, years) | 7.6 | 12.2 | -0.6 | -0.2 |
| F9: ALL-CAPS shouting | 34.6 | 37.1 | -0.5 | -0.3 |
| F10: Quoted speech | 5.9 | 12.2 | -0.9 | -0.4 |
| F11: +n_chars +vader_pos +nominalisation +n_org | 6.8 | 11.4 | -1.1 | -0.3 |
| F12: +first_pl +future_will +modal | 81.9 | 73.4 | 0.3 | -0.1 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 5.5% (3.6%); outrage 59.4% (63.9%); humor 0.7% (0.2%); question 26.9% (15.6%); breaking_live 0.4% (2.9%); episode_show 0.4% (2.8%); interview_guest 14.8% (9.8%); reaction 0.4% (2.9%); confrontation 2.2% (7.7%); listicle 0.0% (0.1%); howto_explainer 5.9% (8.0%)


Nearest style neighbors: @BenShapiro [right]; @ThePodcastoftheLotusEaters [right]; @StevenCrowder [right]; @Styxhexenhammer666 [right]; @hutch [neutral]

Nearest topic neighbors: @TheMajorityReport [left]; @DueDissidence [left]; @TheYoungTurks [left]; @clayandbuck [right]; @JillianMichaels [right]


Monthly drift (topic-controlled scores and hook shares; * = partial month):

| month | n | F1 | F2 | F3 | F4 | F5 | F6 | F7 | F8 | F9 | F10 | F11 | F12 | curiosity_gap | outrage | humor |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 2026-01 | 33 | 0.49 | -0.23 | -0.61 | -0.44 | 0.09 | -0.34 | -0.66 | -0.37 | -0.51 | -1.04 | -1.16 | -0.12 | 0.03 | 0.79 | 0.00 |
| 2026-02 | 26 | 0.50 | -0.11 | -0.71 | -0.43 | 0.53 | 0.02 | -0.94 | -0.33 | -0.40 | -1.15 | -1.78 | 0.83 | 0.00 | 0.54 | 0.00 |
| 2026-03 | 33 | 0.36 | -0.75 | -0.30 | -0.19 | 0.41 | -0.58 | -0.47 | -0.77 | -0.50 | -1.22 | -0.79 | 0.10 | 0.09 | 0.52 | 0.00 |
| 2026-04 | 26 | 0.71 | -0.05 | -0.58 | -0.43 | 1.22 | -0.75 | -0.64 | -0.89 | -0.37 | -1.12 | -0.99 | 0.64 | 0.04 | 0.50 | 0.04 |
| 2026-05 | 43 | 0.76 | -0.33 | -0.22 | 0.34 | 0.84 | -0.53 | -0.80 | -0.56 | -0.15 | -0.58 | -0.88 | 0.33 | 0.12 | 0.53 | 0.00 |
| 2026-06 | 38 | 0.37 | -0.43 | -0.60 | -0.40 | 0.87 | -0.23 | -0.84 | -0.87 | -0.56 | -0.95 | -1.19 | 0.77 | 0.00 | 0.58 | 0.03 |
| 2026-07 | 37 | 0.34 | -0.46 | -0.66 | -0.25 | -0.05 | -1.01 | -0.55 | -0.04 | -0.66 | -0.57 | -1.52 | -0.14 | 0.05 | 0.70 | 0.00 |
| 2026-08 | 25 | 1.06 | 0.38 | -0.47 | -0.18 | 1.79 | -0.08 | -0.70 | -0.81 | -0.35 | -0.98 | -0.73 | -0.24 | 0.12 | 0.60 | 0.00 |


Engagement (n = 257, R2 = 0.2546; log views per within-creator SD, month + topic controls):

| predictor | coef_per_sd | se | p |
|---|---|---|---|
| F1 | 0.054 | 0.071 | 0.440 |
| F2 | -0.044 | 0.071 | 0.539 |
| F3 | -0.147 | 0.091 | 0.107 |
| F4 | 0.135 | 0.074 | 0.068 |
| F5 | 0.100 | 0.089 | 0.260 |
| F6 | -0.097 | 0.077 | 0.208 |
| F7 | 0.034 | 0.098 | 0.726 |
| F8 | -0.069 | 0.072 | 0.342 |
| F9 | 0.099 | 0.073 | 0.172 |
| F10 | 0.033 | 0.084 | 0.697 |
| F11 | -0.098 | 0.080 | 0.224 |
| F12 | -0.285 | 0.091 | 0.002 |
| curiosity_gap | -0.100 | 0.080 | 0.210 |
| outrage | 0.042 | 0.086 | 0.630 |
| humor | 0.087 | 0.083 | 0.296 |
| n_tokens | -0.037 | 0.085 | 0.658 |


Hit concentration: Gini 0.6157, top-10 % share 50.7%, power-law tail not supported vs lognormal (LR = 1.0271, p = 0.3044).


Lexical diversity: Heaps' beta (1,500 tokens) 0.8478, Zipf 0.6708, formulaic titles 19.5572 per 100, mean length 7.0996 tokens.


## streams

Titles: 60 rows, 60 unique (repeat share 0.0%); ranked; political share 98.3%.


Top topics: Political Figures and Callouts (6.7%); ICE protests in Minneapolis (5.0%); Hasan Piker Controversy (5.0%); Political Commentary and Guests (5.0%); Kamala Harris 2028 Presidential Run (5.0%)


| dimension | percentile (topic-controlled) | percentile (raw) | score | group median |
|---|---|---|---|---|
| F1: Positive tone vs outrage (shock words, violence verbs, negative sentiment) | 54.5 | 59.7 | 0.1 | -0.2 |
| F2: +has_finite_verb +present_tense +verb_share +past_tense | 20.8 | 16.9 | -1.0 | -0.4 |
| F3: Labelled live/formulaic headline (LIVE:, BREAKING:, colon labels) | 48.1 | 39.0 | -0.2 | -0.2 |
| F4: +trailing_ellipsis +discourse_marker +ellipsis +contraction | 57.1 | 57.1 | -0.1 | -0.2 |
| F5: Question and explainer framing (why, what, ?) | 31.2 | 53.2 | -0.4 | -0.3 |
| F6: Person-centred (named people) | 68.8 | 72.7 | 0.7 | 0.7 |
| F7: Descriptive news prose vs title-case (nouns, adjectives, places) | 45.5 | 29.9 | -0.5 | -0.4 |
| F8: Numeric and dated (digits, years) | 35.1 | 37.7 | -0.1 | -0.1 |
| F9: ALL-CAPS shouting | 54.5 | 61.0 | -0.3 | -0.4 |
| F10: Quoted speech | 41.6 | 37.7 | -0.6 | -0.6 |
| F11: +n_chars +vader_pos +nominalisation +n_org | 59.7 | 53.2 | 0.1 | 0.0 |
| F12: +first_pl +future_will +modal | 29.9 | 35.1 | -0.2 | -0.0 |


Hooks / formats (share of titles; channel-group mean in brackets): curiosity_gap 0.0% (2.6%); outrage 40.0% (62.7%); humor 1.7% (0.1%); question 6.7% (11.3%); breaking_live 0.0% (17.4%); episode_show 28.3% (6.2%); interview_guest 78.3% (18.4%); reaction 0.0% (1.4%); confrontation 1.7% (15.4%); listicle 0.0% (0.0%); howto_explainer 1.7% (2.8%)


Nearest style neighbors: @rolandsmartin [left]; @NewsmaxTV [right]; @thewarningwithsteveschmidt [left]; @TheYoungTurks [left]; @hutch [neutral]

Nearest topic neighbors: @TheMajorityReport [left]; @NewsmaxTV [right]; @RealAmericasVoice [right]; @thewarningwithsteveschmidt [left]; @underthedesknews [left]


Engagement: n < 100 titles with views, not estimated.


Lexical diversity: Heaps' beta (1,500 tokens) None, Zipf None, formulaic titles 18.3333 per 100, mean length 10.7 tokens.
