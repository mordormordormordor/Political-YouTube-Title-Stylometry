...
from pipeline_titles.common import ANALYSIS_DIR, FORMATS_PARQUET, LOW_N, MONTHS, PARTIAL_MONTH, TOPICS_CSV, load_lanes, load_prepared, stage_timer
...
def run(info: dict) -> None:
    ...
    lane_m = ok.groupby(["lane", "genre", "month"]).agg(n_creators=("creator", "nunique"), n_titles=("n_titles", "sum"),
                                                        **{c: (c, "mean") for c in score_cols}).reset_index()
    lane_m["partial_month"] = lane_m["month"] == PARTIAL_MONTH
    ...
    corpus_m.insert(0, "lane", "ALL (mean of creators)")
    corpus_m["partial_month"] = corpus_m["month"] == PARTIAL_MONTH
    pd.concat([lane_m, corpus_m], ignore_index=True).to_csv(ANALYSIS_DIR / "drift_lane_monthly.csv", index=False)
    ...
    tc = pd.DataFrame(rows).merge(lanes, on="creator", how="left")
    ...
    tcl = tc[~tc["low_n"].astype(bool)].groupby(["lane", "genre", "month_to"]).agg(n_creators=("creator", "nunique"), mean_js=("js_distance", "mean"), median_js=("js_distance", "median")).reset_index()
    tcl.to_csv(ANALYSIS_DIR / "topic_change_lane_monthly.csv", index=False)

    # trends: Spearman of month index vs value
    trend_rows = []
    for (lane, g), grp in lane_m.groupby(["lane", "genre"]):
        if len(grp) < 5:
            continue
        x = grp["month"].map(MONTHS.index)
        for c in [f + "_controlled" for f in fcols] + HOOKS:
            r, p = spearmanr(x, grp[c])
            trend_rows.append({"level": "lane", "group": lane, "genre": g, "measure": c, "n_months": len(grp), "spearman_trend": round(float(r), 3), "p": round(float(p), 4),
    ...
    info.update(lane_month_rows=len(lane_m), top30=len(top30), topic_change_rows=len(tc))
