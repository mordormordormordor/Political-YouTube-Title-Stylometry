...
- **The rater.** All ratings and labels behind the style model, the hooks and the formats come from one automated rater at temperature 0. Weighted kappa on a 300-title retest: sensational {float(rt.loc[rt.dimension == 'sensational', 'weighted_kappa'].iloc[0]):.2f}, critical {float(rt.loc[rt.dimension == 'critical', 'weighted_kappa'].iloc[0]):.2f}, analytical {float(rt.loc[rt.dimension == 'analytical', 'weighted_kappa'].iloc[0]):.2f}, conversational {float(rt.loc[rt.dimension == 'conversational', 'weighted_kappa'].iloc[0]):.2f}, educational {float(rt.loc[rt.dimension == 'educational', 'weighted_kappa'].iloc[0]):.2f}; kappa on the outrage flag {float(rt.loc[rt.dimension == 'outrage', 'kappa'].iloc[0]):.2f}, on the format label {float(rt.loc[rt.dimension == 'format_llm', 'kappa'].iloc[0]):.2f}. The validation of the tone factor stands on the reliable ratings; the rest is indicative.
- **The channel groups** come from Claude Opus's reading of 50 sampled titles per channel (document 14). The score is reliable as a ranking (split-half Spearman {float(rd('leaning_split_half.csv').split_half_spearman_mean.iloc[0]):.2f}), but the ±0.05 thresholds are a choice, {int(rd('leaning_stability.csv').group_changed.iloc[0])} channels changed group between the first 16-title draw and the full sample, and a group says how a channel's titles *read*, not what it is. Every group-level table changes if the thresholds or the judge change; the creator-level tables, neighbors and clusters do not.
