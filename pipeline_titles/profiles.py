"""Stage 6 - channel profiles behind the question documents (09-13):
    ...
    style_twins.csv         every (left commentary, right commentary) pair with its distance
                            in z-scored topic-controlled style space; style_twins_nearest.csv
                            gives each creator's nearest cross-divide twin and how that
                            distance ranks among all its neighbours
    outrage_by_lane_ci.csv  outrage-frame share per lane x genre: mean of creators with a
                            bootstrap 95 % CI (1,000 resamples), median, quartiles
...
from pipeline_titles.common import ANALYSIS_DIR, DIMENSIONS_CSV, FEATURES_TITLE, FORMATS_PARQUET, LOW_N, SEED, load_lanes, load_prepared, stage_timer
...
def run(info: dict) -> None:
    ...
    uniq = uniq.merge(lanes, on="creator", how="left")
    ...
    cp = cp.merge(lanes, on="creator", how="left").sort_values(["genre", "caps_any"], ascending=[True, False])
    ...
    ar = ar.merge(lanes, on="creator", how="left")
    ...
    kw = pd.DataFrame(krows).merge(lanes[["creator", "lane"]], on="creator", how="left")
    kw.to_csv(ANALYSIS_DIR / "signature_keywords.csv", index=False)

    # ---- stylistic twins across the divide (videos, ranked, non-clipper) ----
    ...
    X = Z.to_numpy(); names = d["creator"].tolist(); lane_of = dict(zip(d["creator"], d["lane"]))
    D = np.sqrt(((X[:, None, :] - X[None, :, :]) ** 2).sum(axis=2))
    all_pairs = D[np.triu_indices(len(names), 1)]
    left = [i for i, c in enumerate(names) if lane_of[c] == "left_commentary"]
    right = [i for i, c in enumerate(names) if lane_of[c] == "right_commentary"]
    prow = []
    for i in left:
        for j in right:
    ...
    for i in left + right:
        ...
        nrow.append({"creator": names[i], "lane": lane_of[names[i]], "twin_across_divide": names[j], "twin_distance": float(D[i, j]),
                     "twin_rank_among_all_neighbours": rank, "nearest_same_lane": names[nearest_same] if nearest_same is not None else "",
                     "nearest_same_lane_distance": float(D[i, nearest_same]) if nearest_same is not None else np.nan,
                     "twin_closer_than_any_same_lane": bool(nearest_same is not None and D[i, j] < D[i, nearest_same])})
    ...
    # ---- outrage by lane with bootstrap CI ----
    sh = pd.read_csv(ANALYSIS_DIR / "format_hook_shares.csv")
    cr = sh[(sh["level"] == "creator") & (~sh["low_n"].astype(str).str.lower().eq("true"))]
    rng = np.random.RandomState(SEED)
    orow = []
    for (lane, genre), g in cr.groupby(["lane", "genre"]):
        v = g["outrage"].to_numpy(dtype=float)
        boots = [rng.choice(v, size=len(v), replace=True).mean() for _ in range(1000)] if len(v) > 1 else [v.mean()]
        orow.append({"lane": lane, "genre": genre, "n_creators": len(v), "mean": v.mean(), "ci95_low": np.percentile(boots, 2.5), "ci95_high": np.percentile(boots, 97.5),
                     "median": np.median(v), "q25": np.percentile(v, 25), "q75": np.percentile(v, 75), "min": v.min(), "max": v.max()})
    pd.DataFrame(orow).sort_values(["genre", "mean"], ascending=[True, False]).to_csv(ANALYSIS_DIR / "outrage_by_lane_ci.csv", index=False)
