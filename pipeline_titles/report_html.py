...
function renderCard(){
  ...
  if(c.leaning) h += `<h3>Political leaning from titles (two models, ${c.leaning.n_titles} sampled titles)</h3><p>mean score <b>${fmt(c.leaning.mean_score)}</b> (−1 = every title reads left, +1 = every title reads right) → <b>${esc(c.leaning.implied_side)}</b>; ${Object.entries(c.leaning).filter(([k])=>!['mean_score','implied_side','n_titles','consensus_neither'].includes(k)).map(([k,v])=>`${esc(k)} ${fmt(v)}`).join(' · ')}; both models say 'neither' on ${fmtPct(c.leaning.consensus_neither)} of agreed titles</p>`;
...
function rowsAll(){
  ...
  return creators.map(c=>{ const k=D.creators[c], g=k.genres[genre]; if(!g) return null;
    const r={creator:c, name:k.channel_name, lane:k.lane, organisation:k.organisation, clipper:k.clipper, n_unique:g.n_unique, low_n:g.low_n, political:g.political_share,
      ...
      arousal:g.arousal?g.arousal.index:null, caps:g.caps_profile?(g.caps_profile.all_caps+g.caps_profile.selective_caps):null, leaning:k.leaning?k.leaning.mean_score:null};
